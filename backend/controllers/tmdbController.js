...
const getTrending = async (req, res) => {
  ...
  if (!validTimes.includes(time)) return res.status(400).json({ error: "Invalid time window" });

  const data = await tmdb.get(`/trending/${type}/${time}`, {}, tmdb.TTL.trending);
  res.json(data);
};
...
const search = async (req, res) => {
  ...
  const data = await tmdb.fetch(`/search/${type}`, { query: q, include_adult: false });
  res.json(data);
};
